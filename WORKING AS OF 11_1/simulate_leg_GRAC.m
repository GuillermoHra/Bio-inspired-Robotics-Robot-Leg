...
%      D_x = 4;  % Damping X
%      D_y = 4;  % Damping Y
%      rEd = [0 0 0]'; % Desired position of foot
     %omega=3;%3,30
     %rEd=[0.025*cos(omega*t);-0.125+0.025*sin(omega*t)];
     %rEd=[0.025*cos(omega*t);-0.23+0.025*sin(omega*t)];
     %r_E= position_foot(z, p);
     %r_E_dot=velocity_foot(z,p);
     %J=jacobian_foot(z,p);
     %taup=[K_x*(r_E(1)-rEd(1))+D_x*(r_E_dot(1)-0);K_y*(r_E(2)-rEd(2))+D_y*(r_E_dot(2)-0)]
     %tau1=-J'*taup

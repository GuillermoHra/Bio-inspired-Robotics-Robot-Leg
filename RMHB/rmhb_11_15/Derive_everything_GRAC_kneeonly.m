...
%ermhat =  sin(thm-thk+thh)*ihat + -cos(thm-thk+thh) * jhat;
...
%rcm4=ra+lc4*erahat;
%rm=ra+.2*ermhat;
keypoints = [rcm1 rh0 rk ra];% rm];  %rm for motor angle
...
matlabFunction(C,'file',[directory 'kC_' name],'vars',{z u p});
matlabFunction(dC,'file',[directory 'kdC_' name],'vars',{z u p});
matlabFunction(ra,'file',[directory 'kposition_foot'],'vars',{z p});
matlabFunction(dra,'file',[directory 'kvelocity_foot'],'vars',{z p});
...
COM = [rcm1(2); drcm1(2)]; % Concatenate  y coordinate and speeds of center of mass in array
matlabFunction(COM,'file',[directory 'kCOM_' name],'vars',{z p});
matlabFunction(Jleg,'file',[directory 'kjacobian_foot'],'vars',{z p});
